...
import { LayoutDashboard, Package, ShoppingCart, Users, LogOut, Menu, X, Plus, Edit2, Trash2, Banknote, Eye, Search, Bell, ChevronDown, TrendingUp, TrendingDown, Clock, CheckCircle2, XCircle, ArrowUpRight, Filter, Settings } from 'lucide-react';
...
const ADMIN_EMAILS = ['yashrajnirdhar007@gmail.com', 'nirdhar007@gmail.com', 'admin@gmail.com'];
    ...
    <div className="min-h-screen flex bg-[#FAF7F0] text-[#242424] font-sans selection:bg-[#234B2A] selection:text-white">
      ...
      <div className={`fixed inset-y-0 left-0 w-64 bg-[#234B2A] text-white z-50 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} shadow-2xl flex flex-col`}>
        <div className="flex items-center gap-3 p-6 border-b border-white/10">
          ...
          <div>
            <h2 className="text-lg font-bold font-serif tracking-wide leading-tight">GHARGHUTI</h2>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="bg-white/80 backdrop-blur-md shadow-sm h-16 flex items-center px-4 md:px-8 border-b border-[#E0E0E0] z-30 sticky top-0">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <button className="relative text-gray-400 hover:text-[#234B2A] transition-colors">
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStats({
          orders: 5, revenue: 1450, products: 5, users: 3,
    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-[#616161]">Here's what's happening with Gharghuti Pickles today, {today}.</p>
        </div>
            ...
            {stats.bestSellers.length === 0 ? (
              ...
              stats.bestSellers.map((prod: any, i) => (
                <div key={prod.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                  <img src={prod.image || 'https://via.placeholder.com/100'} alt={prod.name} className="w-12 h-12 rounded-lg object-cover shadow-sm" />
                  ...
                  products.map(prod => (
                    <tr key={prod.id} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="p-4 flex items-center gap-4">
                        <img src={prod.image || 'https://via.placeholder.com/100'} alt={prod.name} className="w-12 h-12 rounded-lg object-cover shadow-sm border border-gray-100" />
                        <div>
...
const Orders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const ords = snap.docs.map(doc => ({ ...doc.data(), id: doc.id }));
      // Sort desc
      ...
      if (existingOrdersStr) {
        setOrders(JSON.parse(existingOrdersStr));
      }
    } finally {
      setLoading(false);
    }
